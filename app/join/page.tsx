        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:'16px',padding:'28px'}}>
          ...
          {status === 'found' && (
            <>
              ...
              {user ? (
                <button onClick={handleJoin} style={{width:'100%',background:C.gold,color:C.bg,fontWeight:'700',fontSize:'14px',padding:'12px',borderRadius:'8px',border:'none',cursor:'pointer'}}>
                  Join {group?.name}
                </button>
              ) : (
                <>
          ...
          {status === 'error' && (
            <>
              ...
              <button onClick={() => router.push('/')} style={{background:C.gold,color:C.bg,fontWeight:'700',fontSize:'13px',padding:'10px 20px',borderRadius:'8px',border:'none',cursor:'pointer'}}>
                Go to CommunityFlow
              </button>
            </>
          )}
  ...
  return (
    <Suspense fallback={
      <div style={{minHeight:'100vh',background:'#0B1F3A',display:'flex',alignItems:'center',justifyContent:'center'}}>
        <div style={{color:'#D4A017'}}>Loading...</div>
      </div>
    }>
      <JoinContent />
    </Suspense>
